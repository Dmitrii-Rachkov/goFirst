...
// Суть интерфейса Reader в том, что в нем уже где-то хранятся данные, мы передаём
// в функцию Read(p []byte) (n int, err error) это хранилище. Сам reader записывает в это хранилище информацию
// и возвращает количество байт, которое записал и ошибку.
// Ниже пример реализации reader
func simpleReader() {
	buf := make([]byte, 10)
...
// Пример как не хранить большое количество данных в одной переменной,
// например когда считываешь данные из большого файла
func rowsReader() {
