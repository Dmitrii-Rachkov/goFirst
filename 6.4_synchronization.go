...
Например, одна горутина производит некоторое действие, результат которой используется в другой горутине.
В этом плане мы можем использовать каналы для синхронизации.
Например, одна горутина вычисляет факториал числа, а результат выводится в другой горутине:
*/

func sync() {
	fmt.Println("Синхронизация горутин")
	intCh := make(chan int)
	go exFactorial(5, intCh)
	fmt.Println(<-intCh)
}

func exFactorial(n int, ch chan int) {
	result := 1
	for i := 1; i <= n; i++ {
		result *= i
	}
...
/*
Канал не обязательно должен нести данные, которые представляют некоторый результат, от которого
зависит дальнейшее выполнение горутины.
Иногда это может быть холостой объект, например, пустая структура, которая необходима только
...
После закрытия канала в функции main получае соответствующий сигнал в строке <-structCh,
и после этого функция main продолжает свою работу.
*/
